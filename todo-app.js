(function () {
  ...
  function createTodoItem(name, done = false) {
    ...
    item.classList.add(
      ...
      "align-itemsFromLocalStorage-center"
    );
    ...
    localStorage.setItem(
      localStorageName,
      JSON.stringify(Array.from(itemsFromLocalStorage.entries()))
    );
  ...
  function createTodoApp(container, title = "Список дел", arr = []) {
    ...
    function itemDone(todoItem) {
      ...
      itemsFromLocalStorage.set(
        todoItem.item.querySelector("p").textContent,
        todoItem.item.classList.contains("list-group-item-success")
          ? true
          : false
      );
      localStorage.setItem(
    ...
    function listenItemDone(todoItem) {
    ...
    function listenItemDelete(todoItem) {
      todoItem.deleteButton.addEventListener("click", function () {
        if (confirm("Вы уверены?")) {
          todoItem.item.remove();
          itemsFromLocalStorage.delete(
            todoItem.item.querySelector("p").textContent
          );
